...
**Time:** about 45 minutes. Use only this disposable Codespace and source-free files.

<details><summary>Quick start, safety, and reset</summary>

Run `cd exercises/lab-04-skills-memory`; inspect the shipped skill and template before asking an agent to edit. Do not copy private instructor repositories, endpoints, credentials, or restricted data. Reset only `task/` and `template/` after saving your notes; keep the loop bounded and stop at the human review point.
</details>

## What you learn

Put repeated knowledge in the smallest correct place—prompt, project rule, skill, memory/log, or deterministic script—and evaluate whether a lightweight project-control structure helps a fresh agent resume work.
...
6. Test resumption: start a fresh agent context and ask it to continue from the handoff. Compare what it knows with the actual files.
...
The facilitator may show a richer private research-control repository as a contrast: it has more automation, domain machinery, and operational assumptions than this minimal public template. Those private files are not copied here. Compare the systems by evidence and maintenance cost, not by feature count.
...
**TODO (later data-task design session):** define one approved synthetic-panel task for the bounded loop, including exact input, expected artefact, invariant check, and a safe handoff record. Keep the template generic until the generator and instructor reviews are complete; never use real SOEP or private project data.
