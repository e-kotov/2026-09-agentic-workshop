...
# The generator deliberately does not activate a project library or .Rprofile.
# A caller that wants the locked environment supplies R_LIBS_USER (or an
# explicit --library path) before invoking Rscript.
...
output_root <- arg_value("--output-dir", file.path(root_dir, "fixtures"))
explicit_library <- arg_value("--library", Sys.getenv("SYNTH_PANEL_LIBRARY", ""))
if (nzchar(explicit_library)) .libPaths(c(normalizePath(explicit_library, mustWork = FALSE), .libPaths()))

required_packages <- c("fabricatr", "simstudy", "data.table", "digest")
...
hierarchy <- fabricatr::fabricate(
  households = fabricatr::add_level(
    N = n_households,
    household_counter = seq_len(n_households),
...
# Domain IDs are assigned from workshop-owned counters.  The automatic
# fabricatr labels are used only as an internal join scaffold and never leave
# this block, so package formatting/order changes cannot become public IDs.
household_counter <- seq_len(n_households)
person_counter <- seq_len(nrow(hierarchy))
household_lookup <- hierarchy[!duplicated(hierarchy$households), c(
  "households", "household_counter", "region4", "selection_probability"
)]
household_lookup <- household_lookup[order(household_lookup$household_counter), ]
household_lookup$hid <- sprintf("H%04d", household_lookup$household_counter)

person_static <- data.frame(
  pid = sprintf("P%06d", person_counter),
  origin_hid = sprintf("H%04d", hierarchy$household_counter),
...
# Simplified, normalized inverse-selection teaching weights.  These are not
# nonresponse/attrition weights: no wave-constant response factor is divided
# out before normalization, because that would algebraically cancel.
...
long_base <- 1 / hierarchy$selection_probability[match(complete_baseline, person_static$pid)]
...
defect_profiles_path <- file.path(root_dir, "defect_profiles.csv")
if (!file.exists(defect_profiles_path)) {
  stop("Missing generator declaration: ", defect_profiles_path, call. = FALSE)
}
defect_profiles <- utils::read.csv(defect_profiles_path, stringsAsFactors = FALSE,
                                   check.names = FALSE)
if (anyDuplicated(defect_profiles$profile) || any(!nzchar(defect_profiles$profile))) {
  stop("defect_profiles.csv must have unique non-empty profiles", call. = FALSE)
}
profile_failure_set <- function(profile) {
  row <- defect_profiles[defect_profiles$profile == profile, , drop = FALSE]
  if (!nrow(row)) stop("No defect profile declaration for ", profile, call. = FALSE)
  value <- row$intended_failed_checks[[1]]
  if (is.na(value) || !nzchar(value)) character() else strsplit(value, "\\|", fixed = FALSE)[[1]]
}

lockfile_path <- file.path(root_dir, "renv.lock")
if (!file.exists(lockfile_path)) {
  stop("Missing generator-scoped renv.lock: ", lockfile_path, call. = FALSE)
}
lockfile_sha256 <- digest::digest(file = lockfile_path, algo = "sha256", serialize = FALSE)
generator_sha256 <- digest::digest(file = script_path, algo = "sha256", serialize = FALSE)
generator_input_files <- c("defect_profiles.csv", "invariants.csv", "dependencies.csv", "renv.lock", "renv-bootstrap.csv")
if (!all(file.exists(file.path(root_dir, generator_input_files)))) stop("Missing generator input declaration file", call. = FALSE)
generator_input_hashes <- vapply(generator_input_files, function(name) digest::digest(file = file.path(root_dir, name), algo = "sha256", serialize = FALSE), character(1))
generator_inputs_sha256 <- digest::digest(paste(generator_input_files, generator_input_hashes, sep = "=", collapse = "\n"), algo = "sha256", serialize = FALSE)
package_version <- function(pkg) as.character(utils::packageVersion(pkg))
...
write_release <- function(release_tables, release_dir, profile, stale_checksum = FALSE) {
  ...
  manifest <- do.call(rbind, lapply(names(release_tables), function(table_name) {
    ...
    data.frame(
      release_id = paste0("synthetic-panel-", generator_version),
      ...
      fabricatr_version = package_version("fabricatr"),
      simstudy_version = package_version("simstudy"),
      data_table_version = package_version("data.table"),
      digest_version = package_version("digest"),
      renv_version = if (requireNamespace("renv", quietly = TRUE)) package_version("renv") else "not_loaded",
      lockfile_sha256 = lockfile_sha256,
      generator_sha256 = generator_sha256,
      generator_inputs_sha256 = generator_inputs_sha256,
      source_provenance = "source-free: no respondent/source records are generator inputs",
      table_name = table_name,
      file_name = file_name,
      row_count = nrow(release_tables[[table_name]]),
      sha256 = digest::digest(file = path, algo = "sha256", serialize = FALSE),
      expected_validation_status = if (profile == "clean") "pass" else
        paste0("expected_failure:", paste(profile_failure_set(profile), collapse = "|")),
      target_total = if (table_name == "person_wave") 10000 else
        if (table_name == "household_wave") 5000 else NA_real_,
      target_unit = if (table_name == "person_wave") "invented person population units per wave" else
        if (table_name == "household_wave") "invented household population units per wave" else "",
      stringsAsFactors = FALSE
    )
  }))
...
write_csv(questionnaire, file.path(root_dir, "questionnaire.csv"))
fixtures_dir <- output_root
if (normalizePath(fixtures_dir, mustWork = FALSE) != normalizePath(file.path(root_dir, "fixtures"), mustWork = FALSE)) {
