...
not SOEP estimates or a statistically calibrated replica of SOEP.
...
weights are simplified inverse-probability teaching constructs scaled to
invented totals; they are not valid SOEP weights. Imputations are deterministic
within-wave teaching substitutions, not a production imputation method.

## Files and regeneration

- `generate.R` builds `fixtures/clean/` and six named one-change releases in
  `fixtures/defects/` from seed `20260912`.
...
- `validate_all.R` confirms that the clean release passes and every defect
  release fails its intended check.
- `schema.csv` is the data dictionary. `questionnaire.csv` is wave-specific
  ...
  SHA-256 hashes, generator/package versions, and invented target totals.

The checked-in fixture was built with the versions in `dependencies.csv`.
Install the R packages into any reversible user/project library, for example:

```sh
SYNTH_PANEL_R_LIB="$(mktemp -d)"
R_LIBS_USER="$SYNTH_PANEL_R_LIB" Rscript -e \
  'install.packages(c("fabricatr", "simstudy", "digest"), repos="https://cloud.r-project.org")'
```

Then make that same library visible when running the scripts. Exact generated
bytes are supported for the recorded versions; later compatible package
versions should preserve the documented schema and invariants but may draw
...
- `release_manifest.csv`: release provenance, counts, hashes, and check status.
